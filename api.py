...
@app.route('/listar_arquivos/<username>', methods=['GET']) # lista todos os arquivos que começam com o prefixo do usuário
def listar_arquivos(username):
    arquivos = [f for f in os.listdir(UPLOAD_FOLDER) if f.startswith(username + "_")]
...
def salvar_dados():
    ...
    conn = conecta_db()
    cursor = conn.cursor()
    cursor.execute("SELECT UserName FROM informacoes WHERE UserName = %s", (data['UserName'],))
    if cursor.fetchone():
        cursor.close()
        conn.close()
    ...
    hashed_password = generate_salted_hash(data['PasswordUser'])
    ...
    try:
        ...
        copiar_arquivos_padrao(data['UserName'])
...
def copiar_arquivos_padrao(username): # copia os arquivos da pasta dos arquivos padrões e coloca um prefixo (usuário)
    ...
    if os.path.exists(pasta_padrao):
        for arquivo in os.listdir(pasta_padrao):
            ...
            if os.path.isfile(origem):
                destino = os.path.join(app.config['UPLOAD_FOLDER'], f"{username}_{arquivo}")
...
def login():
    ...
    cursor.execute("SELECT * FROM informacoes WHERE UserName = %s", (username,)) # retorna todos os dados da tabela (inclusive a senha hash+salt)
    user = cursor.fetchone() # se não achar o que procura, não retorna nada
    cursor.close()
    conn.close()

    if user and verify_password(user['PasswordUser'], password): # checa se user existe e pega a senha digitada (password) e confere com o hash+salt armazenado
        user.pop('PasswordUser', None)  # tira a senha do dicionário do usuário antes de devolver
        return jsonify({ # mensagem de sucesso
...
def deletar_conta():
    ...
    try:
        ...
        cursor.execute("DELETE FROM informacoes WHERE UserName = %s", (username,))
        conn.commit()
        # apaga arquivos do usuário que foram salvos
        arquivos_usuario = [f for f in os.listdir(UPLOAD_FOLDER) if f.startswith(username + "_")] # faz uma listagem e pega todos os arquivos que começam com o nome do usuário
...
def resetar_arquivos():
    ...
    if not username: # mensagem de erro
        return jsonify({'mensagem': 'Usuário não especificado.', 'status': 'erro'}), 400
    try:
        # deleta os arquivos atuais do usuário
        arquivos_usuario = [f for f in os.listdir(UPLOAD_FOLDER) if f.startswith(username + "_")]
        for arquivo in arquivos_usuario:
            try:
        ...
        copiar_arquivos_padrao(username)
        # mensagem de sucesso
